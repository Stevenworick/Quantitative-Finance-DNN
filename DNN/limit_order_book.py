...
class LimitOrderBook:
    ...
    __all_limit_orders_array: np.array
    ...
    def __init__(self, curr_pair: CurrPair):
        ...
        self.__all_limit_orders_array=None
    
    def limit_bids(self):
        return self.__limit_bids
    
    def limit_offers(self):
        return self.__limit_offers
    ...
    def on_new_order(self, quote: Quote):
        ...
        liste_temp=[quote.price(),quote.amount(),quote.id(),quote.time(),quote.way(),quote.type()]
        if self.__all_limit_orders_array is None and len(self.limit_bids())>5 and len(self.limit_offers())>5:
            self.__all_limit_orders_array = np.array(liste_temp)
        elif self.__all_limit_orders_array is not None:
            self.__all_limit_orders_array=np.vstack([self.__all_limit_orders_array,liste_temp])

    def initialize_order_book_array(self):
        self.__all_limit_orders_array=None
